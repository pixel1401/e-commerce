...
export function NavBar() {
    ...
    const { user, cartState } = useProviderState();
                        ...
                        : (
                            <Col xs='auto'>
                                <Stack direction='horizontal'>
                                    ...
                                    <Link style={{ margin: '0 25px 0px 15px ', position: 'relative' }} to={RouterPath.cart}>
                                        <Icon icon={shoppingCart} size={20} />
                                        <div style={{ width: '20px', height: '20px', borderRadius: '50%', background: 'orange', position: 'absolute', top: '-8px', right: '-13px', textAlign: 'center', fontSize: '12px' }} >{cartState.length}</div>
                                    </Link>
